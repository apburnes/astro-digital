...
function rowPathOperator(operator, row, path) {
  var rowNumber = validateNumber(row);
  var pathNumber = validateNumber(path);
  ...
  var rowNum = buildRowPath('row', row);
  var pathNum = buildRowPath('path', path);
  ...
  var query = wrapParens({query: rowWithPath});
...
function xy(x, y) {
  var validX = validateCoordinate(180, x);
  var validY = validateCoordinate(90, y);

  if (!validX || !validY) {
    this._error = new Error('Searching an xy requires valid Latitude and Longitude');
  }

  var query = buildExtent(x, y, x, y);
...
function bbox(xMin, yMin, xMax, yMax) {
  var validXMin = validateCoordinate(180, xMin);
  var validYMin = validateCoordinate(90, yMin);
  var validXMax = validateCoordinate(180, xMax);
  var validYMax = validateCoordinate(90, yMax);

  if (!validXMin || !validYMin || !validXMax || !validYMax) {
    this._error = new Error('Searching a bbox requires valid Latitudes and Longitudes');
  }

  var query = buildExtent(xMin, yMin, xMax, yMax);
...
function buildExtent(xMin, yMin, xMax, yMax) {
  var upperY = buildRange('upperLeftCornerLatitude', [yMax, 1000]);
  var lowerY = buildRange('lowerRightCornerLatitude', [-1000, yMin]);
  var lowerX = buildRange('lowerLeftCornerLongitude', [-1000, xMin]);
  var upperX = buildRange('upperRightCornerLongitude', [xMax, 1000]);

  var query = upperY.concat('+AND+', lowerY, '+AND+', lowerX, '+AND+', upperX);
