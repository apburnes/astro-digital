...
function cloudCover(range) {
  var coverRange = [_.min(range), _.max(range)];
  var querystring = buildRange(coverRange, 'cloudCoverFull');

  this.query = this.query.concat(querystring);
  return this;
}

// function aquisitionDate(range) {
//   var querystring = buildRange(range, 'aquisitionDate');
